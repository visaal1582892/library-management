...
public class ViewAllMembersController {
	...
	@FXML
	private TableColumn<MemberTableView, MemberGender> memberGenderColumn;
	@FXML
	...
	public void initialize() throws SQLException {
		...
		try {
			...
			for (Member member : members) {
				memberData.add(new MemberTableView(member.getMemberId(), member.getMemberName(), member.getMemberMail(),
...
//		deleteColumn
//				.setCellFactory(new Callback<TableColumn<MemberTableView, Void>, TableCell<MemberTableView, Void>>() {
//					@Override
...
//										"-fx-background-color: #ef5350; -fx-text-fill: white; -fx-font-weight: bold; -fx-background-radius: 6;");
//								deleteButton.setOnAction(event -> {
//									MemberTableView member = getTableView().getItems().get(getIndex());
//									try {
//										boolean deleted = new MemberServiceImplementation()
//												.deleteMemberById(member.getMemberId());
//										if (deleted) {
//											getTableView().getItems().remove(member);
//											ResponseHandler.showResponse(message, "Member Deleted Successfully...",
//													Color.GREEN);
//										} else {
//											ResponseHandler.showResponse(message, "Failed to Delete Member...",
//													Color.RED);
//										}
//									} catch (DatabaseException e) {
